...
建商展示用前端 app — 數位孿生 Dashboard，iOS app 質感，Metaarchetech Design System。

作為 Visustwin 平台的主展示介面，整合來自 WebController（OSC Bridge via WebSocket `ws://localhost:9000`）與 welltek-twin（MQTT Bridge via `ws://localhost:3001`）的即時資料。無後端時自動 fallback 至 mock 資料，可獨立展示。
...
- **rename**：demo 後執行 metaarchetech → VisTwin 全域 rename，影響此 repo 的 GitHub remote URL

## 連結

- GitHub：[metaarchetech/visustwin-showcase](https://github.com/metaarchetech/visustwin-showcase)
