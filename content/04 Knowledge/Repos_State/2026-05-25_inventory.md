...
date: 2026-05-25
host: visustwin (D:\Visustwin)
total_repos: 16
git_repos: 16
non_git_project_folders: 2
pat_scope_note: 當前 gh PAT 為 fine-grained，只看得到 metaarchetech-vault / metaarchetech.github.io / vistwinsite。其餘 8 個 metaarchetech repo API 端 404（PAT 沒授權該 repo），本機 git fetch 用 cached 憑證也 403。Remote 欄位有些只能從 local cache 推。
---
...
對接戰略：[[../../Now|Now]] / [[../../00 System/Entity_Map|Entity_Map]]。
...
註：fetch 對 8 個 metaarchetech repo 因 PAT 沒授權回 403，這些 repo 的 "up-to-date" 是依本機最後一次成功 fetch 的 cache 判斷，不是即時 truth。obsidian / metaarchetech-site / vistwinsite / NVIDIA kit-app-template / anthropic-cookbook / claude-skills / founder-os 是真即時。
...
- 屬於[[../../01 Projects/visustwin/project_kit_web_architecture|kit-web architecture]]主舞台
- **動作**: 決定要不要 rebase 到 NVIDIA 110.1.1；本機腳本是否移到[[../../01 Projects/visustwin/project_kit_bridge|kit-bridge]] `_repl/` 之外的地方
...
- 已被 [[../../01 Projects/visustwin/yolo_next_steps|visustwin-vision + visustwin.vision.detector]] 取代
...
- 1 個 `cmd.py`
- [[../../01 Projects/visustwin/project_kit_bridge|kit-bridge]] file IPC 落腳處
- 不需動，已記憶
...
- 遠端: **public**、archived=false、open_issues=1、最近 push 2026-04-22、開 PR #1 `claude/rebrand-audit` Rebrand audit: VisusTwin → VisTwin
