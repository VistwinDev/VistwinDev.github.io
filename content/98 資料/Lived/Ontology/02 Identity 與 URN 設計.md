...
---
tags: [ontology, identity, urn, iri, VisTwin]
created: 2026-04-17
updated: 2026-04-17
---
...
VisTwin 現在一個「主臥」在三個子系統裡的身份:

| 系統 | 身份形式 | 範例 |
|---|---|---|
| Omniverse USD | prim path | `/World/SampleHome_A/F2/Room_MBR` |
| MQTT | topic 片段 | `visustwin/siteA/showroom/f2/mbr` |
| welltek-twin SQLite | PK integer | `rooms.id = 42` |

問題很清楚:
- 你無法只憑其中一個找到另外兩個
- 每改一個系統的 naming rule,另外兩個要手動同步
- 新加一個子系統(例如 ArchVizExplorer 的 OSC address)又多一套
...
```
urn:visustwin:{site}:{building}:{unit}:{room}:{entity_type}:{entity_id}
```
...
| 實體 | URN |
|---|---|
| 示範案場 A | `urn:visustwin:siteA:_:_:_:site:siteA` |
...
每個 prim 的 `customData` 必有 `visustwin:urn` key:

```python
prim.SetCustomDataByKey("VisTwin:urn", "urn:visustwin:siteA:buildingA:showroom:mbr:room:MBR")
prim.SetCustomDataByKey("VisTwin:class", "Room")
```

詳見 [[06 USD customData 約定]]。
...
urn:visustwin:siteA:buildingA:showroom:mbr:sensor:lux-007
    ↓ (把 `:` 換成 `/`,去掉 `urn:` 前綴)
VisTwin/siteA/buildingA/showroom/mbr/sensor/lux-007
```

有 payload 時:

```
Topic:   VisTwin/siteA/buildingA/showroom/mbr/sensor/lux-007/state
...
### OSC (ArchVizExplorer / WebController)
...
/VisTwin/light/hue-007/brightness → 0.6
```

若要全限定,用完整 URN:

```
/VisTwin/entity  "urn:visustwin:siteA:...:hue-007"  0.6
```
...
舊系統(MQTT topic、welltek 既有資料、USD 既有場景)不可能一夜換完。分三階段:

### Phase 1:只規格化,新系統強制 (現在)

- 寫死 URN 規則到 ontology doc
- 新寫的 extension、新 migration、新 data ingestion **一定用 URN**
- 舊系統維持不動

### Phase 2:加 URN 欄位 (3 個月內)
...
未來 `visustwin_ontology/core.py` 會提供:

```python
from visustwin_ontology import URN
...
# 轉 topic / osc
urn.to_mqtt_topic()   # "VisTwin/siteA/buildingA/showroom/mbr/sensor/lux-007"
urn.to_osc_address()  # "/VisTwin/sensor/lux-007"
...
你的東西不是 API,是實體。房間不會有 v2,就是搬走或拆掉 → 發一個新 URN、舊的標 `deprecated: true`。

---

## 📝 相關連結

- [[01 T-Box vs A-Box]] — URN 是把 T-Box 與多個 A-Box 黏起來的膠水
- [[06 USD customData 約定]] — USD 側 URN 寫入細節
- [[05 VisTwin T-Box v1 類別清單]] — 每個類別的 URN pattern
