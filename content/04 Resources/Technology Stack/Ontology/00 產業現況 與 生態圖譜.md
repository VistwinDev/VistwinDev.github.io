...
---
tags: [ontology, industry-landscape, standards, visustwin]
created: 2026-04-17
updated: 2026-04-17
---
...
| 領域 | 事實標準 | 主管機構 | 誰在用 | 對 VisusTwin |
|---|---|---|---|---|
| **BIM / 建築設計** | IFC + ifcOWL | buildingSMART | Autodesk Revit、ArchiCAD、政府採購 | 建商給 .rvt / .ifc 時接 |
| **智慧建築營運** | Brick Schema | UC Berkeley / LBNL | Google、Microsoft、Siemens 商辦 | **樣品屋感測器、HVAC、照明最實用** |
...
- **對 VisusTwin:** 不直接用,但建商若給 IFC 模型,我們要有能力萃取空間拓撲(Building → Storey → Space)
...
- **對 VisusTwin:** 第一順位對齊對象。我們的 `visustwin:Sensor` 在 docstring 標注 `≈ brick:Sensor`
...
- **對 VisusTwin:** 配 Brick 用,`Observation` 的概念直接借來當 runtime telemetry payload
...
- 歐盟部分法規會引用
- **對 VisusTwin:** 台灣建商案子暫不急,但若接 Matter / Thread / HomeKit 家電孿生值得看
...
- **雲鎖定**:離開 Azure 幾乎沒生態
- **對 VisusTwin:** 不用。但它的文件寫得非常好,值得當**學習範例**看「雲廠商怎麼做孿生 ontology」

### OpenUSD

- 嚴格講**不是 ontology**,是**場景描述 + schema 系統**,但提供 ontology 能掛進來的底座
- 你可以用 **IsA API schema** 自定語意類別,或輕量用 **customData dict** 塞 metadata
- **對 VisusTwin:** 第一版走 customData(見 [[06 USD customData 約定]]),穩定後考慮升級 IsA schema

### Schema.org

- Google / Bing / Yahoo 為 SEO 發起,描述網頁結構化內容
- 與建築孿生沒啥關係,但如果你想讓**樣品屋展示頁**被 Google 吃出豐富結果卡,可加 `RealEstateListing` 型別
- **對 VisusTwin:** 周邊加值,非核心
...
- **CityGML** — 城市級 GIS,OGC 標準。適合做**都市級**孿生,不是單棟樣品屋。
...
1. 建築師交 `.rvt` / `.ifc` → 你轉 USD 進 Omniverse(**幾何**)
2. 你自己小 ontology 標註「這是主臥、這是樣品建材選項 A」(**語意**)
3. 感測器走 Brick / 自家 MQTT(**運行時**)
4. welltek-twin 存供應商產品目錄(**商業**)

大型標準對 VisusTwin 是「**需要時再對齊**」的目標,不是「一開始就全吃」的起點。

---

## 🧭 VisusTwin 的對齊策略

| VisusTwin 類別 | 結構對齊 | namespace 歸屬 |
|---|---|---|
| `Site` / `Building` / `Floor` / `Unit` / `Space` | BOT | `visustwin:` (自有) |
| `Sensor` / `Actuator` / `Observation` | Brick + SOSA | `visustwin:` (自有) |
| `SampleHome` / `ShowcaseFeature` / `MaterialOption` | *(無對應,VisusTwin 獨有)* | `visustwin:` |
| `POI` / `Route` / `ExhibitionBoard` | *(無對應)* | `visustwin:` |
| `WindScenario` / `SolarScenario` | 鬆散對齊 SOSA Procedure | `visustwin:` |

**寫法慣例**:Python class docstring 第一行註明 `Aligns with: bot:Space, brick:Room`,但類別本身繼承 `visustwin.ontology.core.Entity`。未來需要 OWL export 時,rdflib serializer 補 `rdfs:subClassOf` 即可。
...
- [[01 T-Box vs A-Box]]
- [[03 對齊既有標準]] — 本文是概覽,那篇是深入到欄位對應
- [[Omniverse x Revit 2026]] — IFC → USD 具體流程
