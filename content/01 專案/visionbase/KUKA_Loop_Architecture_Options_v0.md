...
date: 2026-04-29
project: VisionBase
author: Frncs (with AI assist)
audience: B (執行) + Frncs (拍板)
...
這**不是 lock 方案的決策文件** — 是**選項菜單 (menu of options)**。

等使用者回答 4 個關鍵問題後再 lock：
(1) KUKA 型號 (2) sensor (3) 第一個情境 (4) sim vs real。

對象：**B 工**（執行）+ **Frncs**（戰略拍板）。
...
- **B 銜接點**：Grasshopper 已能處理點雲 / 幾何，RGB-D → Rhino 是熟悉路徑
...
- **B 銜接點**：Grasshopper 已處理過影像 → 幾何，這層是它的「升級版」
- **典型起手**：YOLO + Foundation Pose 已能應付大多數 pick task；要走 LLM-based reasoning 才需 VLA

### 1.3 Planning — 路徑規劃

- **選項**：MoveIt 2 (ROS)、cuMotion (NVIDIA GPU 加速)、Isaac Manipulator、**KUKA.PRC (B 已會)**、KUKA 內建 PTP/LIN
- **B 銜接點**：**這是 B 最強的銜接點** — KUKA.PRC 從 Grasshopper 出 path 是熟練操作。可組合：PRC 出粗 path → cuMotion 做避障 fine-tune
- **典型起手**：Pick & Place 走 Isaac Manipulator 內建 cuMotion；複雜建築 toolpath 走 KUKA.PRC

### 1.4 Actuation — 機器人執行層

- **選項**：**KRL (B 已會)**、RSI (Robot Sensor Interface, real-time)、FRI (Fast Research Interface, iiwa 才有)、EKI-XML (網路通訊, KRC4/5)、ROS 2 driver
- **B 銜接點**：KRL 是 B 的 home turf
...
| 系列 | 範例型號 | Controller | Isaac 整合方式 | 對 B 友善度 | 備註 |
|---|---|---|---|---|---|
| KR Agilus / Cybertech | KR3 R540, KR6 R900, KR16 R2010 | KRC4/KRC5 | EKI-XML + Python bridge | ★★★★ KRL 為主 | lab/小空間最常見 |
| KR Quantec | KR60-120, KR210 | KRC4/KRC5 | EKI-XML / RSI | ★★★★ | 工業大型，建築場景對應 |
...
| KR.AI (NVIDIA合作) | Agilus AI 預載 Jetson | KRC5 + Jetson | **官方支援，最簡** | ★ 取得性未知 | 2024 公布，可能難買 |

**敏感性 (要使用者回答的)**：

- 若 lab 是 **KR Agilus + KRC4/5** → 走 EKI-XML，B 上手快，Isaac 整合中等
- 若是 **iiwa** → FRI 真實時控制，是「Isaac 原生支援」最深的，但 B 要學 Sunrise/Java
- 若是 **KR.AI** → 路徑最短但實際取得性 / 預算可能擋路
...
| # | 場景 | 動作描述 | 技術難度 | 寶舖相關度 | Demo 殺傷力 | 預估時程 (B + AI assist) |
|---|---|---|---|---|---|---|
| 1 | **Pick & Place 積木** | 視覺辨識 → 抓 → 放定點 | 低 | 低 | 中 | 3-4 週 |
| 2 | **跟隨人手** | 持續追蹤手部 → 末端跟隨 | 低-中 | 中 | 中 | 3-4 週 |
| 3 | **看圖建實體模型** | 讀 BIM 局部 → 用磚塊堆疊還原 | 中 | 高 | **高** | 6-8 週 |
| 4 | **缺陷標記** | 偵測牆面缺陷 → 末端噴頭標位置 | 中-高 | **極高** | **極高** | 8-12 週 |
| 5 | **BIM 對齊驗證** | 真實場景 vs BIM → 標差異 | 高 | **極高** | **極高** | 10-14 週 |
...
- 失敗模式: 抓不穩、漏判方向。可控
- 評語: B 第一週熱身好題目，但對 demo 不殺、與寶舖故事弱連結
...
- 失敗模式: 累積誤差、堆疊穩定性
- 評語: **B 的 PRC 經驗最對胃口**，視覺加分多。寶舖能感受到「BIM → 實體」的 narrative
...
- 評語: **直接呼應寶舖 WELL Platinum 品質要求**。建築工地真實 use case。風險：要採缺陷資料 train
...
- 評語: **VisTwin 核心戰略最對應**。但工程量大，v1 太重

**不下結論**：5 個場景各有特色。請使用者依下列 3 個維度選：

1. 想對誰 demo？(寶舖老董 / NVIDIA / 投資人)
2. B 是否能在 6-8 週內看到實體成果？
3. 寶舖案要不要直接打？
...
| 對 B 學習價值 | 高 (NVIDIA 主流) | 低 (KRL 已會) | 高 |
...
- **Sim-first**：給 NVIDIA / 投資人 demo「我們已掌握 Isaac 棧」。但寶舖老董會問「實際能動嗎」
- **Real-first**：寶舖老董 / 工地展示。但 v1 學到的東西難複製到第二台機器
- **Hybrid (Sim 跑通邏輯 + Real fine-tune)**：工程最佳，成本最高。**v1 起來最慢，但 v2 之後會補回來**

---

## 6. 我還缺什麼決策（要 Frncs/B 回答的清單）

依重要性排序，**前 5 題是必答**：

### 必答 5 題

1. **KUKA 型號 + Controller + B 目前用的 SDK？**
   直接決定 §2 走哪一列、§1.4 actuation 用 KRL/RSI/FRI/EKI 哪個。

2. **5 個場景中，第一個 loop 要選哪個（或變形）？**
   決策因素 weight：寶舖 demo 殺傷力 vs B 學習曲線 vs 6-8 週可達性。
...
4. **Hardware 預算 ceiling？**
   ...
   影響 sensor 選擇與 §4 場景可行性。

5. **第一個 demo 目標時點 + 對象？**
   NVIDIA 拜訪？投資人 round？寶舖老董？反推時程。

### 次要 3 題

6. **B 全職還是 part-time？** — 直接乘除時程
7. **Lab 空間：KUKA 在哪？電源、安全圍欄、網路？** — 影響 real-first 可行性
8. **是否要為 v1 預留 v2 scale path？** — 影響是否強推 hybrid（短痛長爽）

---

## 附錄 A：Default 路徑（如果 Frncs 懶得選）

> **預設組合**：KR Agilus (KRC4/5) + Realsense D455 + 場景 #3「看圖建實體模型」+ Hybrid (Sim 跑邏輯 → Real fine-tune)

理由：
- 用上 B 最強牌（KUKA.PRC + Grasshopper）
- BIM → 實體的故事直接接寶舖
- 6-8 週可看到實體 demo
- Sim 階段讓 B 學 Isaac，real 階段給殺傷力
- 不需採購工業級 LIDAR / 力控末端，預算可控

**注意**：這只是 sensible default，不是推薦。等 Frncs 回完前 5 題再 lock。

---

## 附錄 B：B 已會 vs 要學

| 元件 | B 現狀 | First loop 要補什麼 |
|---|---|---|
| KUKA.PRC | 熟練 | 與 Isaac path 整合（USD ↔ Rhino） |
| Grasshopper | 熟練 | 接點雲 → 幾何 pipeline |
| KRL | 熟練 | EKI-XML / RSI 從 PC 端送指令 |
| Python | 不確定 | Isaac SDK / OpenCV / YOLO 推理 |
| ROS 2 | 不確定 | 部署階段需要（Isaac ROS） |
