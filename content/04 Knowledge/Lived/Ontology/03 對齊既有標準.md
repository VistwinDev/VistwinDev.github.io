...
---
tags: [ontology, standards, bot, brick, sosa, ifc, dtdl, saref, visustwin]
created: 2026-04-17
updated: 2026-04-17
---
...
> [[00 產業現況 與 生態圖譜]] 是鳥瞰圖,這篇是**逐欄位對照表**。目的:讓 VisusTwin 類別 docstring 能正確標註 `Aligns with: bot:Space`,未來要 export OWL 時不出洋相。
...
| BOT IRI | 中文 | VisusTwin 對應 |
|---|---|---|
| `bot:Zone` | 區域(抽象父類) | `Entity` (我們的 root) |
| `bot:Site` | 案場 / 基地 | `Site` |
...
BOT 是 IFC 的「簡化版拓撲子集」。若要完整對應 BIM,改用 ifcOWL — 但複雜度飆升十倍。VisusTwin 樣品屋案子 **不需要 IFC 複雜度**,BOT 夠用。
...
| `Room(type="MasterBedroom")` | `brick:Bedroom` (Brick 細分度不如 visustwin) |

### 借用策略

- **不要**全盤繼承 Brick 類別樹(它太細,你的樣品屋用不到 100 種 Equipment)
- **借用** point/sensor kind 命名:`brick:Illuminance_Sensor` → VisusTwin 寫 `kind="illuminance"`(改 snake_case)
...
兩個拼起來 = 完整的 smart building 感測模型。VisusTwin 採同樣拆法。

### VisusTwin 的 Observation
...
- **不在 T-Box 階段對齊**
- 當建商給 `.ifc` 檔時,用 IfcOpenShell 萃取 → 映射到 BOT → 進 VisusTwin T-Box
- 只對齊到 **Site/Building/Storey/Space** 這幾個頂層類別,以下深度用 VisusTwin 自有類別
- IFC 的 MEP (機電管路)、結構分析細節 VisusTwin 暫不吃
...
**結論**:VisusTwin **不優先對齊 SAREF**。未來若接 Matter/Thread 家電,只需 export 時多加一組 rdfs 對映。
...
- 客戶要求 Azure Digital Twins 整合時 — 寫一層 VisusTwin → DTDL export
...
- **適合都市孿生**,不是單棟樣品屋
- VisusTwin 未來若做社區 / 多案場整合可考慮

### Schema.org (RealEstateListing)

- 不是 ontology,是搜尋標記
- 建商網站 SEO 用,VisusTwin 可產出 JSON-LD 給 web demo 用
...
- VisusTwin **不主動對齊**,但若接 FIWARE broker 有現成 schema 可借
...
- [[00 產業現況 與 生態圖譜]]
- [[05 VisusTwin T-Box v1 類別清單]] — 每個類別都會標 `Aligns with: ...`
- [[Omniverse x Revit 2026]] — IFC → USD 流程
