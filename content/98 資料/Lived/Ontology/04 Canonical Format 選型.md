...
---
tags: [ontology, format, pydantic, linkml, json-schema, owl, VisTwin]
created: 2026-04-17
updated: 2026-04-17
---
...
> 「用什麼語言寫 ontology?」這問題沒有唯一答案。取捨要看團隊、使用場景、外部交換需求。這篇比較主流選項,並記錄 VisTwin 為什麼選 **Pydantic**。
...
```json
{
  "$id": "https://visustwin.com/schemas/Room.json",
  "title": "Room",
  "type": "object",
  "properties": {
    "urn": { "type": "string", "pattern": "^urn:VisTwin:" },
    "class": { "const": "Room" },
...
class Room(Space):
    ...
    floor_urn: URN
    area: float = Field(..., ge=0, description="Floor area in m²")
...
```yaml
id: https://visustwin.com/linkml/core
name: visustwin-core
prefixes:
  bot: https://w3id.org/bot#
  brick: https://brickschema.org/schema/Brick#
  VisTwin: https://visustwin.com/ns/
...
- 只在 Omniverse 環境有用,welltek / WebController 吃不到
- 改 schema 要重 build
...
|---|---|---|---|---|---|
| Runtime 驗證 | ❌ | ✅ (via validator) | ✅ | ✅ (生成 Pydantic) | ✅ (USD 端) |
| 語意網 / OWL export | ✅ 原生 | ❌ | ❌ (要手寫) | ✅ 原生 | ❌ |
| JSON Schema export | ❌ | ✅ 原生 | ✅ 一行 | ✅ 原生 | ❌ |
| Python 型別 / IDE | ❌ | ✅ (生成) | ✅ 原生 | ✅ (生成) | ✅ (生成) |
| 跨語言 | ✅ | ✅✅ | ⚠️ 透過 JSON Schema | ✅✅ | ❌ |
| 學習曲線 | 高 | 低 | 低 | 中 | 高 |
...
└── *.py                 ← SOURCE OF TRUTH (Pydantic)
        ↓
     (Python import)     Omniverse extensions (VisTwin.*)
        ↓
   model_json_schema()
        ↓
      schemas/*.json     ← welltek-twin / WebController 用
        ↓
    (未來) rdflib
        ↓
      visustwin.ttl      ← 若要對外發布 OWL 時
```
...
import json
from visustwin_ontology.spatial import Site, Building, Floor, Unit, Space
from visustwin_ontology.device import Sensor, Actuator, Observation
# ... all classes
...
- [[05 VisTwin T-Box v1 類別清單]] — 實際 Pydantic 程式碼會長什麼樣
- [[06 USD customData 約定]] — Pydantic 模型如何與 USD 配合
