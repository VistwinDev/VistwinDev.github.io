...
---
tags: [ontology, description-logic, t-box, a-box, visustwin]
created: 2026-04-17
updated: 2026-04-17
---
...
> Ontology 世界最基本的分層概念,來自 **描述邏輯 (Description Logic, DL)**。搞懂這兩個詞,你就能判斷「這件事該放 ontology 檔,還是 USD / SQLite / MQTT 當下狀態」。
...
我們的世界裡,A-Box 不是單一儲存,而是 **多系統視角**:

```
                 T-Box (單一真相)
                 D:\Visustwin\ontology\
                          |
        +-----------------+-----------------+
        |                 |                 |
...
| Runtime (MQTT) | topic `visustwin/siteA/showroom/f2/mbr/light_001/state` → `{ on: true, brightness: 0.6 }` | 秒級,消失 |

三個 A-Box 要「對得上」 → 靠 **URN** 黏合(見 [[02 Identity 與 URN 設計]])。
...
**VisusTwin 照這條路走**。想做 A-Box 整合時,未來用 Oxigraph (嵌入式 RDF store) 當輕量方案,不要一開始就立基在它上面。
...
2. **改這個會影響幾個人?** 只影響我的本週工作 → A-Box。影響其他工程師 / 其他項目 → T-Box。
...
- [[00 產業現況 與 生態圖譜]]
- [[02 Identity 與 URN 設計]]
- [[05 VisusTwin T-Box v1 類別清單]]
