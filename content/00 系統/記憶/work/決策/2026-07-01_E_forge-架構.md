---
title: E_forge 核心架構決策(蒸餾)
type: decision
app: work
created: 2026-07-01
updated: 2026-07-01
tags: [work, 決策, 架構, E_forge]
---

# E_forge 核心架構決策

> 從 2026-06-30~07-01 建置討論蒸餾。完整文件在 `D:\VisTwin\E_forge\core\`。

## 命題
E_forge 不是「網站優化工具」,是用架構治 agent 兩弱點:**①要催促 → n8n 強制迴圈(自主器官)**;**②記憶差 → vault 經驗庫(記憶器官)**。兩者咬合才成飛輪(有迴圈無記憶=瞎忙;有記憶無迴圈=不動)。E_forge = harness,讓 agent 不必自己擅長驅動與記憶。
...
## MCP 框架(全 CLI/HTTP)
MCP = JSON-RPC over stdio/HTTP。E_forge 自開一個 MCP server(TS SDK + HTTP)暴露 recall_experience/read_rubric/dispatch_run/list_agents/stream…;Console 與 worker 用 `--mcp-config` 帶工具(headless `claude -p` 預設 `--bare` 不繼承 `.mcp.json`)。`claude mcp serve` 可把 Claude 自己當 server。
