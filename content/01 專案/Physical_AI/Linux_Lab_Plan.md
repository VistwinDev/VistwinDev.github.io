...
依 [[../../00 系統/Entity_Map|Entity Map]]，VisTwin 戰略為**雙軌平行同級**，不是「主線 + 養現金流支線」：
...
- 移植後 VisTwin vault 與 VisionBase vault 的分工
- 是否 cross-link（Obsidian remote vault sync？）

⏳ 等使用者後續提供詳細需求

> 註：[[../../00 系統/Entity_Map|Entity Map]] 行動項已記「Obsidian vault 資料夾 `metaarchetech` → 改名 `vistwin`」。此跨 vault 移植與該 rename 是兩件事，釐清時需一併釐清兩者先後關係。
...
- [[../../00 系統/Entity_Map|Entity Map]] — 實體邊界與雙軌定義（canonical）
