...
4. **相關 process**：列 VisTwin 相關的 python / node process
   （`Get-Process python,node` 後比對 cmdline / 路徑含 VisTwin）。
5. **Vault 同步**：對 `D:\Visustwin\obsidian` 比對本地 HEAD 與 `origin/master` hash，
   標示 vault 是否落後 / 領先。
6. **車隊**：讀 `99 封存/機台/*.md` 的 frontmatter，列每台機器
   `role` + `last_check_in`。
7. **組成報告**，三段 markdown：
...
- [[../../99 封存/機台/_fleet-index|Fleet Index]]
