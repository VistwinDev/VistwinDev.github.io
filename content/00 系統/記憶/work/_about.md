---
title: Work 超級 app(工作型 · E_forge)· 命名空間入口
type: memory-core
app: work
updated: 2026-07-01
...
# work · 工作型超級 app(E_forge)

> Windows 上的「工作 / 執行型」超級 app,專案碼 `D:\VisTwin\E_forge`。
> 框架定位見 [[Work|超級 App/Work]]。此資料夾是它的記憶命名空間(現在為真 + 指針)。

## 現在為真(2026-07-01)
- **E_forge** = Work 節點的超級 app。命題:用架構治 agent 兩弱點(**要催促 → n8n 強制迴圈**;**記憶差 → vault 經驗庫**)。
- **已跑**:
...
## 指針
- 架構決策 → [[2026-07-01_E_forge-架構]]
- 本次建置情節 → [[2026-07-01_E_forge-建置]]
...
- E_forge 用自己的 repos(待補),勿與 Manager 的 `vistwin-app`/`vistwin-automation` 混。
