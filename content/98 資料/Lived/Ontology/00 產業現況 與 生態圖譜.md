...
---
tags: [ontology, industry-landscape, standards, VisTwin]
created: 2026-04-17
updated: 2026-04-17
---
...
- **對 VisTwin:** 不直接用,但建商若給 IFC 模型,我們要有能力萃取空間拓撲(Building → Storey → Space)
...
- **對 VisTwin:** 第一順位對齊對象。我們的 `visustwin:Sensor` 在 docstring 標注 `≈ brick:Sensor`
...
- **對 VisTwin:** 配 Brick 用,`Observation` 的概念直接借來當 runtime telemetry payload
...
- 歐盟部分法規會引用
- **對 VisTwin:** 台灣建商案子暫不急,但若接 Matter / Thread / HomeKit 家電孿生值得看
...
- **雲鎖定**:離開 Azure 幾乎沒生態
- **對 VisTwin:** 不用。但它的文件寫得非常好,值得當**學習範例**看「雲廠商怎麼做孿生 ontology」

### OpenUSD

- 嚴格講**不是 ontology**,是**場景描述 + schema 系統**,但提供 ontology 能掛進來的底座
- 你可以用 **IsA API schema** 自定語意類別,或輕量用 **customData dict** 塞 metadata
- **對 VisTwin:** 第一版走 customData(見 [[06 USD customData 約定]]),穩定後考慮升級 IsA schema
...
大型標準對 VisTwin 是「**需要時再對齊**」的目標,不是「一開始就全吃」的起點。

---

## 🧭 VisTwin 的對齊策略

| VisTwin 類別 | 結構對齊 | namespace 歸屬 |
|---|---|---|
| `Site` / `Building` / `Floor` / `Unit` / `Space` | BOT | `visustwin:` (自有) |
| `Sensor` / `Actuator` / `Observation` | Brick + SOSA | `visustwin:` (自有) |
| `SampleHome` / `ShowcaseFeature` / `MaterialOption` | *(無對應,VisTwin 獨有)* | `visustwin:` |
