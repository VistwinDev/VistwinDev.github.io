...
---
tags: [ontology, t-box, classes, visustwin, implementation]
created: 2026-04-17
updated: 2026-04-17
---

# 📚 VisusTwin T-Box v1 類別清單

> 本篇是 v1 最終類別階層與欄位定義。階段 2 實作時,Pydantic 程式碼依此表一對一展開。
...
| 欄位 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `urn` | `URN` | ✅ | 全域唯一身份(見 [[02 Identity 與 URN 設計]]) |
| `class_` | `str` | ✅ | 類別名,配合 JSON 序列化的 `class` 鍵 |
| `display_name` | `str` | ✅ | 展示用名稱,可含中文 |
| `description` | `str \| None` | | 長描述 |
| `tags` | `list[str]` | | 自由標籤 |
| `created_at` | `datetime` | ✅ | 首次建立 |
...
| `area` | `float \| None` | | 面積 m²(若適用) |
| `volume` | `float \| None` | | 體積 m³(若適用) |

### Site

- **Aligns with:** `bot:Site`, `IfcSite`
- **語意:** 案場 / 基地(建案)。
- **URN:** `urn:visustwin:{site_slug}:_:_:_:site:{site_slug}`
- **特有欄位:** `coordinate: GeoPoint | None`(WGS84),`address: str | None`

### Building

- **Aligns with:** `bot:Building`, `IfcBuilding`
- **URN:** `urn:visustwin:{site}:{building_slug}:_:_:building:{building_slug}`
- **特有欄位:** `floor_count: int`, `year_built: int | None`

### Floor

- **Aligns with:** `bot:Storey`, `IfcBuildingStorey`
- **URN:** `urn:visustwin:{site}:{building}:_:_:floor:{floor_slug}`(floor slug 建議 `F1`, `F2`, `B1`)
- **特有欄位:** `level_index: int`(樓層數字,地下為負),`elevation: float`(公尺)

### Unit

- **Aligns with:** 無直接對應 (VisusTwin 獨有;IFC 有 `IfcSpatialZone` 但不緊貼)
- **語意:** 樓層裡的「戶」— 一棟樓可切成 A 戶 B 戶,樣品屋專用
- **URN:** `urn:visustwin:{site}:{building}:{unit_slug}:_:unit:{unit_slug}`
...
- **URN:** `urn:visustwin:{site}:{building}:{unit}:{room_slug}:room:{room_slug}`
- **Room 特有欄位:**
...
| 欄位 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `sensor_urn` | `URN` | ✅ | 發出觀測的 sensor |
| `target_urn` | `URN \| None` | | 觀測對象(如房間);省略則代表 self |
| `observed_property` | `str` | ✅ | 對應 sensor.kind |
| `result` | `float \| str \| bool` | ✅ | 觀測值 |
| `unit` | `str \| None` | | SI 單位 |
| `ts` | `datetime` | ✅ | 觀測時間 |
...
## 🏠 Showcase 分支 (VisusTwin 獨有)

### SampleHome

- **Aligns with:** 無 (VisusTwin 獨有)
- **語意:** 一間完整樣品屋(對應 Unit with `unit_type="showroom"`)
- **URN:** `urn:visustwin:{site}:{building}:{unit}:_:sample-home:{slug}`
- **特有欄位:**
...
- **特有欄位:**
  ...
  - `supplier_urn: URN | None`(連回 welltek-twin 供應商)
...
- **語意:** 「一次分析的參數設定」— **不是分析結果本身**。結果檔(PNG、JSON)指標留在 extension 本地(例如 `visustwin.wind.analysis/output/`)
- **欄位:**
...
- **語意:** 3D 場景中的展板(對應 `visustwin.exhibition.board` extension)
- **特有欄位:**
  - `placed_in_urn: URN`
...
```
D:\Visustwin\ontology\visustwin_ontology\
...
- [[02 Identity 與 URN 設計]] — URN 規則
- [[03 對齊既有標準]] — 類別對齊細節
- [[04 Canonical Format 選型]] — Pydantic 實作方式
- [[06 USD customData 約定]] — 類別如何在 Omniverse 場景標註
- [[07 Semantor 的 Ontology 角色]] — Semantor 怎麼使用這套類別
