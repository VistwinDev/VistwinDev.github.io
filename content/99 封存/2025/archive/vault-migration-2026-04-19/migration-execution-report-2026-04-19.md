...
### 1. 路徑型 `[[01 專案/Visustwin/MOC|Visustwin MOC]]` — 18 處
...
### 2. 路徑型 `[[01 專案/OTA120/MOC|OTA120 MOC]]` — 2 處
...
### 4. `[[01 專案 MOC]]` / `[[02 Areas MOC]]` / `[[03 Products MOC]]` / `[[04 Resources MOC]]` / `[[05 Archive MOC]]` — 全部已刪除（obsolete PARA nav）
...
```yaml
aliases: [VisTwin Extensions MOC]
```
...
3. 18 個 plugin 的 `← [[01 專案/Visustwin/MOC|...]]` backlinks 可用腳本批次取代（建議使用者手動執行，以免誤改）。
...
7. `Products/Visustwin/README.md`（原 VisTwin MOC）的標題行 `# Visustwin MOC` 建議改為 `# Visustwin` 以配合新定位（不再是 PARA MOC，而是產品頁）。
