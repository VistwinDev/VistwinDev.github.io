...
- **new** `daily-weekly-journal` (audience: lab, status: active) — 每日/每週把真實工作落進既有 sprint 系統（不另開 `02 Daily/`）讓 Quartz 看到節奏 skill。同建 `02 技術/Visustwin/sprints/2026-W21/` + 週 README + 首篇 `2026-05-20` daily（節奏自 W18 後斷 3 週重啟），並刷 `Now.md`。沿用既有 `Daily Standup` / `Weekly Review` 模板，不建新模板。
...
- **init** — 建立 skill 系統最小起點：`00 系統/技能/`、`SKILL_TEMPLATE.md`、`_index.md`、`_changelog.md`。
- **new** `dev-state-overview` (audience: lab, status: active) — 跨機開發狀態總覽 skill。
- **infra** 建立 `99 封存/機台/` + `_fleet-index.md` + `register-machine.ps1` / `heartbeat.ps1` 多機 heartbeat 機制。
