...
lastUsed: 2026-05-20
confidential: false
revisionTrigger: sprint 資料夾位置遷移 / 改用 Obsidian Daily Notes plugin / vault 從 metaarchetech rename 為 vistwin / 模板大改
...
（`02 Products/Visustwin/sprints/`），讓 Obsidian + Quartz 站點看得到開發節
...
| 項 | 既有位置 | 動作 |
|---|---|---|
| Daily 模板 | `00 System/Templates/Daily Standup.md` | **沿用**，不建新 Daily_Template |
| Weekly 模板 | `00 System/Templates/Weekly Review.md` | **沿用**，不建新 Weekly_Template |
| Sprint 慣例 | `02 Products/Visustwin/sprints/README.md` | 依此 ISO 週命名 |
| Daily 落點 | `02 Products/Visustwin/sprints/2026-W{NN}/YYYY-MM-DD.md` | 寫這裡 |
| Weekly 落點 | `02 Products/Visustwin/sprints/2026-W{NN}/retro.md` 或 `Weekly Review · YYYY-MM-DD.md` 同資料夾 | 寫這裡 |
| 週索引 | 同 sprint 資料夾 `README.md` | 維護該週概覽 |
| 站點曝光 | Quartz 自動吃 vault content；`Now.md` 是 live 焦點頁 | daily 寫完同步刷 `Now.md` |

> **不要**新增 top-level `02 Daily/`：`02` 已是 `02 Products`，會撞號且是另
...
2. 讀 [[working-with-frncs/SKILL|working-with-frncs]] §0–§11（特別 §2 不開戰場、
   §8 事實先於擔憂、§7 真實價值＝進度）。
3. 讀 `02 Products/Visustwin/sprints/README.md` 確認 scrum 慣例沒變。
...
1. ISO 週 → sprint 資料夾 `02 Products/Visustwin/sprints/2026-W{NN}/`；
...
2. **抓真實活動**（有什麼抓什麼，沒有就誠實留空，不要編）：
   ...
   - `05 Claude Skills/_changelog.md` 今天那段（新 / 改 skill）
   - `00 System/Entity_Map.md` 的 `last_updated`（今天有訪談 / 決議？）
   - 跨機狀態可借 [[../dev-state-overview/SKILL|dev-state-overview]]
   - 使用者口述的加註（若有）
3. 套 `00 System/Templates/Daily Standup.md` 結構寫
...
2. 套 `00 System/Templates/Weekly Review.md` 寫 `Weekly Review ·
...
- **跨 sprint 邊界大事**（如 visionbase 進駐）：sprints/README 已明示這
  正常，不要為對齊邊界硬切。
- **命名仍是 metaarchetech**：vault / GitHub org rename 是 Entity_Map 另
  列的本週行動項，**不在本 skill 範圍**。新寫的內容用 VisTwin 命名，但
  不順手去改既有歷史檔的 metaarchetech 連結（§2）。
...
## 返回
- [[Now]] · [[02 Products/Visustwin/sprints/README|sprints]]
```
...
- [[../../02 Products/Visustwin/sprints/README|sprints scrum 慣例]]
- [[../../00 System/Templates/Daily Standup|Daily Standup 模板]]
- [[../../00 System/Templates/Weekly Review|Weekly Review 模板]]
