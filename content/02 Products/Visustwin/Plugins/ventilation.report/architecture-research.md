---
created: 2026-04-29
tags: [project/visustwin, plugin/ventilation.report, research]
---

# Phase 1 — 既有 report 插件架構分析

研究日期:2026-04-29
研究目的:scaffold `visustwin.ventilation.report` 之前,先把既有 report 插件的架構摸透,確保新插件對齊。

---

## 找到的 report 插件(2 個)

| Ext | 用途 | 對應 sim plugin |
|---|---|---|
| `visustwin.solar.report` | 太陽熱能分析報告(SUN 家族) | `visustwin.solar.heatmap` |
| `visustwin.wind.analysis` | 風場 + 行人舒適度報告(AIR 家族) | `visustwin.warp.windtunnel` |

兩者結構**幾乎完全鏡像**。`solar.report` 的 README 明確寫:「結構完全對齊 `visustwin.wind.analysis`」、「Mirror of visustwin.wind.analysis」。**`solar.report` 是較新版本**(命名更整潔 — `xx.report`)且多了 CSV export — **建議當作 canonical reference**。
...
```
visustwin.<subject>.report/                    ← solar.report 用這個
visustwin.<subject>.analysis/                  ← wind.analysis 是舊命名(不要學)
...
└── visustwin/<subject>/report/   或   visustwin/<subject>/analysis/
...
```toml
[package]
title       = "Visustwin <Subject> Report"
description = "<中文一句話描述> · 對齊 ... 的獨立報告模式"
version     = "0.1.0"
category    = "visustwin"
authors     = ["metaarchetech"]
repository  = "https://github.com/metaarchetech/visustwin-extensions"

[dependencies]
"omni.usd"                  = {}
"omni.kit.uiapp"            = {}
"omni.kit.pipapi"           = {}              ← 用來自裝 matplotlib
"visustwin.<sim_plugin>"    = {}              ← 對應 sim plugin

[[python.module]]
name = "visustwin.<subject>.report"
```
...
class XReportExtension(omni.ext.IExt):
    def on_startup(self, ext_id):
        ...
        # 加進 Window 選單
        self._menu_path = "Window/Visustwin <Subject> Analysis"
...
async def _generate_async(self):
    ...
    try:
        ...
        from visustwin.<sim>.<module> import ...    # ← lazy import sim plugin
        # ...load...
        ...
        if not _ensure_charting_deps(self._set_status):
            return
        await app.next_update_async()
        from visustwin.<subject>.report.engine import generate_report
    ...
    Returns:
        {
            "paths": {                         # ← 給 ReportWindow 用,key = tab id
                ...
                "summary":     "<output_dir>/summary.json",     # 不算圖,但放進來
            },
            "stats": { ... }                   # ← summary.json 內容,給 stats bar 直接讀
        }
    """
...
class ReportWindow:
    def __init__(self, image_paths, stats, output_dir, title_info=""):
        ...
        # title_info: 顯示在 header 的副標(例如 "MEDIUM / DAILY / glass 60%")
```
...
- visustwin.<sim_plugin> — 重用 ...
- numpy, matplotlib (Agg backend)

## Related

- visustwin.<sim_plugin> — sim 端
- visustwin.<other>.report — 鏡像 report ext
```
...
兩個 report ext 都已註冊在 `visustwin.dashboard.extension`(line 28-105):

```python
_DESCRIPTIONS = {
    ...
    "visustwin.solar.report":   "太陽熱能分析報告 · 地面熱圖 · 陰影 · 單戶分析 · 獨立視窗",
    "visustwin.wind.analysis":  "風場分析 · 行人舒適度評估",
}

_NAMES_EN = {
    "visustwin.solar.report":  "SOLAR ANALYSIS",
    "visustwin.wind.analysis": "WIND ANALYSIS",
}

_CLASS_MAP = {
    "visustwin.solar.report":  "SolarReportExtension",
    "visustwin.wind.analysis": "WindAnalysisExtension",
}

_CATEGORIES = [
    ("AIR — 空氣", [
        "visustwin.warp.windtunnel",
        "visustwin.wind.analysis",     ← AIR
        "visustwin.ventilation",
    ]),
    ("SUN — 陽光", [
        "visustwin.sunlight.studio",
        "visustwin.solar.heatmap",
        "visustwin.solar.report",      ← SUN
    ]),
    ...
]
```

→ **`visustwin.ventilation.report` 應該插進 `"AIR — 空氣"` 分類,排在 `visustwin.ventilation` 之後**(四件套:windtunnel / wind.analysis / ventilation / **ventilation.report**)。

---

## 8. 報告格式 — 不是 PDF / HTML,是「PNG + JSON + CSV + Pop-out viewer」
...
> 沒看到 reportlab / fpdf2 / weasyprint / jinja2。如果你之後要產 PDF,要新增 dependency,跟既有兩個 ext 不一致。**初版維持 PNG+JSON+CSV 對齊,有需要再加 PDF stage 在 engine.py 後面**。
...
|---|---|---|
| `solar.report` | `solar.heatmap` | `from visustwin.solar.heatmap.demo_preset import load_demo_preset`<br>`from visustwin.solar.heatmap.unit_model import build_project_from_preset`<br>`from visustwin.solar.heatmap.gain_calculator import compute_all_units_*, compute_ground_grid_*, _get_sun_position` |
| `wind.analysis` | `warp.windtunnel` | `from visustwin.warp.windtunnel.solver import sample_velocity_grid, sample_velocity_section` |
...
## 11. 為新 plugin 對齊用的 checklist(給 Phase 2 用)

新 ext `visustwin.ventilation.report` 要走的決策:

1. **命名**:`visustwin.ventilation.report`(對齊 solar.report 命名,不要學 wind.analysis 舊命名)
2. **module 路徑**:`visustwin/ventilation/report/`
3. **class 名**:`VentilationReportExtension`
4. **menu path**:`Window/Visustwin Ventilation Analysis`(注意 Wind/Solar 用「Analysis」,不是「Report」)
5. **dependency**:`visustwin.ventilation = {}`
6. **資料源**:從 `visustwin.ventilation.tracer` / `particles` / `scene` import 三個模組(或讀 USD stage 上的 inlet prim 屬性)
...
9. **CSV**:跟 solar.report 一樣加 `inlets.csv`(每個 inlet 一 row,記 center / normal / size / speed),工程閱讀方便
10. **dashboard 註冊**:加在 `_CATEGORIES["AIR — 空氣"]` 第 4 個位置(在 ventilation 後)
    - `_DESCRIPTIONS["visustwin.ventilation.report"] = "新風 / 換氣分析報告 · 進氣口配置 · 流場 · 覆蓋率 · 獨立視窗"`
    - `_NAMES_EN["visustwin.ventilation.report"] = "VENTILATION ANALYSIS"`
    - `_CLASS_MAP["visustwin.ventilation.report"] = "VentilationReportExtension"`
11. **README**:照第 6 節結構寫
12. **目錄命名**:`output/ventilation_<config>_<YYYYMMDD>_<HHMMSS>/`
13. **時間序列圖**(用戶提的):這個沒有 precedent — 既有 5 張都是「靜態空間分析」,沒有「時間序列」。
    - 選項 A:獨立成第 6 張 `06 time_series.png`(粒子總數 / 進入率 / 排出率 vs time)
    - 選項 B:嚴格 5 張,把這個合併進 `04 coverage_heatmap` 變雙 panel(類似 solar.report Chart 5 的「雙 panel」)
14. **多配置比較**(用戶提的):這個現有 ext 都沒做(每次 generate 一個 config)。
    - 建議 Phase 2 scaffold 階段保留為 stub,日後當 Phase 3 加值

### 故意跟 solar.report 不同的部分(請用戶確認)

| 項目 | 既有 | 提議的 ventilation.report 差異 | 原因 |
|---|---|---|---|
| 報告章節 | 純技術圖表 | **加「業主結論段」+ 「工程附錄段」分流(在 report_window 裡用 tab group 區分)** | 用戶要求單一報告同時涵蓋兩種讀者 |
| 圖表數 | 5 | **5-6**(如果加時間序列獨立成第 6 張) | ventilation 有時間維度 |
| 業主向結論 | 沒有 | **新增 `summary_text.md` 進 output 資料夾**,給 report_window 第一個 tab 顯示 | 業主不看圖只看結論 |

如果這些差異 OK,Phase 2 scaffold 就照這個走。
...
1. **5 張圖清單** 是否 OK?時間序列要獨立第 6 張、還是合進 chart 5 雙 panel?
2. **業主結論段** 用什麼形式呈現 — 純文字 `summary_text.md` 顯示在 report_window 第一個 tab?還是直接放在 stats bar 上方?
3. **多配置比較** — 確認 Phase 2 先不做(stub 起來保留 hook)
4. **CSV 要不要**(建議要,對齊 solar.report)
5. **PDF 要不要**(建議**不要** — 跟既有兩個 ext 一致就好,有需要 v0.2 再加)

確認後 Phase 2 動作:scaffold 寫到 `C:\Visustwin\kit-app-template\source\extensions\visustwin.ventilation.report\`,**不 mirror、不 commit**(vault 這份會跟 obsidian-git 一起進)。

---

## 返回

- [[02 Products/Visustwin/Plugins/ventilation/README|ventilation(sim 端)]]
- [[02 Products/Visustwin/Plugins/README|Plugins]]
- [[02 Products/Visustwin/sprints/2026-W18/2026-04-29|2026-04-29 daily log]]
