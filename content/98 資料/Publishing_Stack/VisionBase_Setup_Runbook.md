...
3. 建議的 vault 結構樣板（不含內容）：
   ```
   00 系統/         (entity map / templates / 機器盤點)
   01 專案/
   02 技術/
   03 公司/
   98 資料/
   00 系統/技能/  (lab 共用的 Claude skill 集合)
   99 封存/機台/
   99 封存/
...
> 驗證原則參考 [[../../00 系統/技能/vault-publish-pipeline/SKILL#Goal-backward verification]]：抓站點 URL 內容 grep 改動關鍵字，才算端到端。
