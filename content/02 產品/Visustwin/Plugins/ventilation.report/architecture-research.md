---
created: 2026-04-29
tags: [project/VisTwin, plugin/ventilation.report, research]
---
...
```
VisTwin.<subject>.report/                    ← solar.report 用這個
VisTwin.<subject>.analysis/                  ← wind.analysis 是舊命名(不要學)
...
└── VisTwin/<subject>/report/   或   VisTwin/<subject>/analysis/
...
"VisTwin.<sim_plugin>"    = {}              ← 對應 sim plugin

[[python.module]]
name = "VisTwin.<subject>.report"
```
...
class XReportExtension(omni.ext.IExt):
    def on_startup(self, ext_id):
        ...
        # 加進 Window 選單
        self._menu_path = "Window/VisTwin <Subject> Analysis"
...
async def _generate_async(self):
    ...
    try:
        ...
        from VisTwin.<sim>.<module> import ...    # ← lazy import sim plugin
        # ...load...
        ...
        if not _ensure_charting_deps(self._set_status):
            return
        await app.next_update_async()
        from VisTwin.<subject>.report.engine import generate_report
...
- VisTwin.<sim_plugin> — 重用 ...
- numpy, matplotlib (Agg backend)

## Related

- VisTwin.<sim_plugin> — sim 端
- VisTwin.<other>.report — 鏡像 report ext
```
...
- [[02 產品/VisTwin/Plugins/ventilation/README|ventilation(sim 端)]]
- [[02 產品/VisTwin/Plugins/README|Plugins]]
