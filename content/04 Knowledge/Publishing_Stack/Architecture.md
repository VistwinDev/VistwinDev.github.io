...
> 既有 [[../../05 Claude Skills/vault-publish-pipeline/SKILL|vault-publish-pipeline SKILL]]
...
└──────────────────────────────────────────────────────────────┘
          ...
          ▼
┌──────────────────────────────────────────────────────────────┐
│  [2] GitHub: metaarchetech/metaarchetech-vault (PRIVATE)     │
...
└──────────────────────────────────────────────────────────────┘
          ...
          ▼
┌──────────────────────────────────────────────────────────────┐
│  [3] GitHub: metaarchetech/metaarchetech.github.io (PUBLIC)  │
│      Default branch: v4                                      │
│      Workflows:                                              │
...
01 Projects/      — VisTwin 各專案（Physical AI / BIM / 寶舖 ...）
02 Products/
03 Company/       — VisTwin Technology 法人 / 商標 / 募資相關
04 Knowledge/     — RAG / 技術筆記（本檔所在）
05 Claude Skills/ — 可觸發 skill 集合
06 Machines/      — 機器盤點
99 Archive/       — legacy 文件
...
```
remote: Permission to metaarchetech/metaarchetech.github.io.git denied to metaarchetech.
...
| GitHub org rename（metaarchetech → vistwin-tech） | vault remote URL、DASHBOARD_PAT scope（新 org）、sync-to-quartz.yml 第 2 步的 repository 路徑、site repo 名稱、quartz.config.ts 的 baseUrl、AiChat workerUrl（worker name 含 org 字串）、Plausible 站台名、Pages 重新 build |
...
- [[../../05 Claude Skills/vault-publish-pipeline/SKILL|vault-publish-pipeline]] — 操作 / 診斷 skill
- [[VisionBase_Setup_Runbook]] — 從零部署同樣系統到 VisionBase
- [[../../00 System/Entity_Map|Entity_Map]] — 實體 / org / 主線雙軌邊界
- [[../../05 Claude Skills/working-with-frncs/SKILL|working-with-frncs]] — §13 pre-flight、§15 失敗 stop
