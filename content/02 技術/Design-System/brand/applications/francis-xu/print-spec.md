...
> **字標大小寫**:名片用**混排 `{ VisTwin }`**(非官方全大寫 `{ VISTWIN }`)。此為使用者品牌決定。
> 小寫字母無現成外框,故字母改用系統指定字型 **Space Grotesk 700** 排版(綠括號沿用官方外框路徑);
> 出 PDF 後用 Ghostscript `-dNoOutputFonts` 全轉外框,交付檔一樣零字型依賴(見第五節)。
...
- 中:`{ VisTwin }` 鎖版(混排)— 括號縮至**與大寫同高、上下對齊、左右氣口對稱**
