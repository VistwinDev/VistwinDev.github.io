...
`[攝影機 RTSP串流]          │       ▼ [tapo.py (YOLO Client)]   └─ YOLOv8 偵測/追蹤   └─ OpenCV 抓取畫面   └─ 封包送出:       - 圖片 (JPEG)       - 偵測結果 (JSON)       │       ▼ [Socket Server (Omni Extension)]   └─ 接收圖片並快取本機 temp 資料夾   └─ 解析 detection JSON   └─ 於 Omniverse UI 顯示圖片/標註資訊       │       ▼ [Omniverse Composer/Kit App]`
...
`visustwin_ov/   ├─ tapo.py                # Client: 負責攝影機偵測/推送資料   ├─ visustwin.yolosocket_ui_extension/   │    ├─ extension.py      # Omniverse extension 主程式 (UI 與 server)   │    └─ socket_server.py  # socket server 實作   └─ requirements.txt       # Python 依賴 (client)`
...
（建議補上 `requirements.txt`，方便一鍵安裝）

### Extension 部署（server/Omni 端）

1. 將 `visustwin.yolosocket_ui_extension` 複製到 Omniverse extensions 目錄
