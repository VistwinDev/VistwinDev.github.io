...
**兩頁共通上排**:左 `● 維思孿生科技`(綠點在左);右 = 正面 `vistwin.com ↗`(綠箭頭在右)、
背面小鎖版 `{ VISTWIN }`。兩頁上排同 top、左右軸一致。
...
- 左下(英文身分塊):`Francis Xu` + `Chief Executive Officer`;右下:`+886 905 255 570` / `frncsxu@vistwin.com`;
- 底排下緣與正面 tagline 下緣切齊。
...
- **綠只在**:`{ }` 括號、公司名 `●`、網址 `↗`。**槓:白版綠(補空、當亮點)、黑版灰(深場已飽滿)**;
  白版兩頁槓對齊絕對位置(翻面重合)。
