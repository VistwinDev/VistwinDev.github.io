...
date: 2026-05-20
status: baseline
project: Physical AI（主線 A）— VisionBase lab 執行端
...
> 使用者 2026-05-20 明確：**預算與機器由使用者搞定，本檔只負責軟體架構 + 作業系統 + 之後依使用者提供的硬體架構收斂**。本檔不臆測硬體與預算。

## 0. 文件定位 — 雙軌平行同級

依 [[../../00 System/Entity_Map|Entity Map]]，VisTwin 戰略為**雙軌平行同級**，不是「主線 + 養現金流支線」：

- **主線 A — Physical AI**：ROS2 + KUKA + NVIDIA Isaac + Omniverse，執行端在 VisionBase lab
- **主線 B — BIM + Digital Twin**：與 A 同等級

本檔是 **主線 A 的 lab Linux 機 Phase 0（環境就緒）建置計畫**。Phase 0 完成 = lab 機具備跑通 [[KUKA_Loop_Architecture_Options_v0|第一個 KUKA loop]] 所需的全部軟體基礎。本檔**只到環境就緒**，第一個 loop 場景選擇 / sim-vs-real 路線屬 [[KUKA_Loop_Architecture_Options_v0]] 的決策範圍，不在此重複。

---

## 1. 硬體規格

⏳ **待使用者補**。硬體規格與硬體架構由使用者後續提供；本檔不臆測機型、GPU、記憶體、網路拓撲。

收到硬體架構後需回頭校驗的軟體決策：
- GPU 世代 → NVIDIA driver 分支 + CUDA 版本 + Isaac Sim 最低需求是否滿足
- 是否多機（工作站 + Jetson edge）→ 是否需要 §2.3 的 Isaac ROS / Jetson 部署支線
- 網路是否與 KRC 控制器同網段 → 影響 §2.5 KUKA 介接層的部署位置（參 [[kuka-control-feasibility]] §「接下來執行步驟」第 2 點）
...
| 選項 | 配套 | 生態成熟度 | 建議 |
|---|---|---|---|
| **Ubuntu 22.04 LTS (Jammy)** | ROS2 Humble + Isaac Sim 4.x | Isaac Sim / ROS2 / KUKA ROS driver 主流驗證組合，社群文件最厚 | ✅ **採此為 baseline** |
| Ubuntu 24.04 LTS (Noble) | ROS2 Jazzy | 生態剛跟上，Isaac Sim 對 24.04 支援陸續到位但社群踩雷紀錄仍少 | 暫不用；待 24.04 + Jazzy 生態與 22.04 同厚再評估遷移 |

**建議理由**：第一個 loop 的目標是「最短路徑跑通 sense→reason→act」，不是追新。22.04 + Humble + Isaac Sim 4.x 是 NVIDIA 官方範例、KUKA ROS2 driver、社群教學三方都對齊的組合，踩雷成本最低。24.04 的好處（較新核心 / 套件）對 lab 第一階段沒有 P0 價值，遷移成本應留到主線進入 scale 階段再付。
...
| Isaac Sim | **Isaac Sim 4.x**（Omniverse Kit-based 3D 物理模擬，吃 USD/URDF） | First loop 基本盤（[[KUKA_Loop_Architecture_Options_v0]] §3） |
...
依 [[KUKA_Loop_Architecture_Options_v0]] §3「First loop 推薦組合」：開發階段 Isaac Sim + Isaac Manipulator，v1 不碰 Lab / Cosmos。

### 2.4 USD 工具鏈

- **OpenUSD**（usd-core / usdview）—— USD 檢視與除錯
- **Omniverse USD Composer**（選裝）—— scene 組裝
- B 銜接點：Rhino / Grasshopper（KUKA.PRC）出 path → USD 互通是 B 的關鍵橋（參 [[KUKA_Loop_Architecture_Options_v0]] §1.3、附錄 B）；USD ↔ Rhino 的具體 pipeline 屬 loop 設計細節，不在 Phase 0 範圍

### 2.5 KUKA 介接層

依 [[kuka-control-feasibility]] 既有 protocol 分析，**正常監控與外部控制都可自寫，不需破任何安全機制**。Linux 機這側要準備的介接元件：
...
> ⚠️ **KUKA Office Lite（KRC 模擬器）是 Windows 程式**，不在 Linux 機上跑。Phase 2 離線模擬若要用 Office Lite，需另一台 Windows 機 / VM；Linux 機這側用 Isaac Sim 做物理模擬（兩者分工見 [[kuka-control-feasibility]] §「Isaac Sim 替代分析」）。
...
- [ ] 準備 EKI socket client 骨架 + XML 協定草案（依 [[kuka-control-feasibility]] §「外部控制流程」）
- [ ] ⏳ KRC 端前置（KSS 版本 / 網段 / KCP 鑰匙）—— **lab 現場確認，非 Linux 機軟體任務**（參 [[kuka-control-feasibility]] §「接下來執行步驟」）
- [ ] **驗證**：Linux 機對 KUKA Office Lite 或 Isaac Sim 模擬端跑通一次 EKI 收發（真機收發屬安全紀律 Phase 3+，見 §5）

---

## 4. KUKA 介接協定整理（baseline，沿用 ground-truth）

摘要自 [[kuka-control-feasibility]]（該檔為 canonical，本節僅供本計畫快速參照，不重複維護）：
...
沿用 [[kuka-control-feasibility]] §「Vibe Coding 的安全紀律」。八十萬等級設備，LLM **不主導物理動作流程**：
...
- 主線 A 的 act 端閉環，數位語言是 BIM→USD ontology（[[Physical_AI_Chapter_Draft_v0]]）—— 與主線 B 的 BIM/DT 共用同一條 USD 場景圖
- lab Linux 機產出的 USD scene / KUKA URDF 應可被主線 B 的 Omniverse pipeline 消費，反之 BIM→USD 輸出可餵 Isaac Sim
- Phase 0 不需實作此接口，但 USD 工具鏈（§2.4）選型須保留兩軌互通性，不可鎖死成只服務單軌

---

## 7. 跨 Vault 規劃（新加，placeholder）

使用者 2026-05-20 提及：之後可能會有 **VisionBase 的 Obsidian vault**，現有 vault 部分內容需要移植過去。

待釐清：
- VisionBase vault 在哪台機（lab Linux 機 / 另開 server）
- 哪些內容要移植（VisionBase / Physical_AI / Lived 等資料夾？）
- 移植後 metaarchetech vault 與 VisionBase vault 的分工
- 是否 cross-link（Obsidian remote vault sync？）

⏳ 等使用者後續提供詳細需求

> 註：[[../../00 System/Entity_Map|Entity Map]] 行動項已記「Obsidian vault 資料夾 `metaarchetech` → 改名 `vistwin`」。此跨 vault 移植與該 rename 是兩件事，釐清時需一併釐清兩者先後關係。

---

## 8. 既有 ground-truth 連結

- [[kuka-control-feasibility]] — KUKA 監控/控制可行性 + 5-Phase 安全紀律（canonical）
- [[KUKA_Loop_Architecture_Options_v0]] — 第一個 loop 4 元素架構 + 場景菜單 + sim-vs-real（canonical）
- [[Physical_AI_Chapter_Draft_v0]] — Physical AI 戰略定位（sense/reason/act 閉環、雙軌歸屬）
- [[../../00 System/Entity_Map|Entity Map]] — 實體邊界與雙軌定義（canonical）
...
| ❓ **開放問題（待 loop 決策，不在本檔）** | 第一個 loop 場景、sim-vs-real 路線、sensor 選型 → 全在 [[KUKA_Loop_Architecture_Options_v0]] §6，本檔不複製 |
| ⏳ **等使用者補** | §1 硬體規格/架構 / §2.3 driver+CUDA 版本鎖定 / §2.3 是否含 Jetson(Isaac ROS) / §3 Phase 0e KRC 現場前置 / §7 跨 Vault 規劃全段 |
