---
created: 2026-04-29
tags: [project/visionbase, sprint/2026-W18, inventory]
---

# Visionbase 區網盤點(`192.168.0.0/24`)

掃描時間:2026-04-29 14:22 UTC(`scan.json` `scannedAt`),掃描耗時 17 秒。
共 **34 台裝置**,**29 在線 / 5 離線**。

資料源:
- `C:\Visustwin\visionbase-monitor\.cache\scan.json`(PowerShell 掃描 + enrich,有 ownership / category)
- `C:\Visustwin\network-scan\devices.json`(較早一輪的 raw 結果,vendor 字串較完整,用來補欄位)

> 備註:Personal 類(個人手機 / 隨機 MAC)的 MAC 略保留前 3 段,其餘照實列。

---

## 基礎建設(Infrastructure / VB 公司財產)
...
詳見 [[01 Projects/visionbase/kuka-control-feasibility|kuka-control-feasibility]]。

---

## 3D 印表機(疑似 Bambu Lab,VB 公司財產 / 待確認)
...
| 192.168.0.178 | `F4:26:79:D8:2A:8E` | Intel Corporate | `DESKTOP-VISIONBASE09.local` | Windows PC | ✅ | VB |
| 192.168.0.220 | — | — | `DESKTOP-JDT5BJ8` | Windows PC(本機) | ✅ | metaarchetech 開發機 |
...
| 192.168.0.152 | `18:4A:53:20:11:C4` | Apple, Inc. | `VISIONBSEdeMini` | macOS / iOS Device | ✅ | VB(hostname 含 VISIONBASE) |
...
- [[01 Projects/visionbase/README|Visionbase]]
- [[01 Projects/visionbase/visionbase-monitor|visionbase-monitor]]
